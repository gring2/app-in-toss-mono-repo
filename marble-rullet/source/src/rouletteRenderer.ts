...
export class RouletteRenderer {
  ...
  protected _theme: ColorTheme = Themes.light;
  ...
  render(renderParameters: RenderParameters, uiObjects: UIObject[]) {
    ...
    // Draw dot-grid pattern to differentiate from flat canvas
    this.ctx.fillStyle = 'rgba(120, 120, 120, 0.15)';
    const dotSpacing = 32;
    const dotRadius = 1.5;
    for (let x = 0; x < this._canvas.width; x += dotSpacing) {
      for (let y = 0; y < this._canvas.height; y += dotSpacing) {
        this.ctx.beginPath();
    ...
    // Make the lines significantly thicker to look like a physical toy track
    this.ctx.lineWidth = 14 / (renderParameters.camera.zoom + initialZoom);
