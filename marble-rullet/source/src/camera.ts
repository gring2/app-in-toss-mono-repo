...
export class Camera {
  ...
  private _calcTargetPositionAndZoom(marbles: Marble[], stage: StageDef, needToZoom: boolean, targetIndex: number) {
    ...
    if (marbles.length > 0) {
      ...
      // Look slightly ahead of the marble vertically (downward)
      const lookAheadY = targetMarble.position.y + 2.0;

      this.setPosition({ x: targetMarble.position.x, y: lookAheadY });

      if (needToZoom) {
        const goalDist = Math.abs(stage.zoomY - this._position.y);
        // Tighter minimum zoom to feel better on small smartphone screens
        this.zoom = Math.max(1.8, (1 - goalDist / zoomThreshold) * 4);
      } else {
        // Higher base zoom for standard smartphone reading
        this.zoom = 1.35;
      }
    } else {
      this.zoom = 1.35;
